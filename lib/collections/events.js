...
Events.after.insert( function(userId, doc) {
  ...
  // Make sure a maximum of one notification exists (don't keep creating new
  // ones if the user spams the like/unlike toggle)
  if ( !Notifications.find(notificationAttrs) ) {
    Notifications.insert(notificationAttrs);
  }
});
